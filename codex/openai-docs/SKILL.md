...
- If OpenAI docs MCP tools are not visible in the current tool list, use tool discovery when available before falling back to web search or install guidance.
...
For answers or edits driven by this skill, verify the source path used:

- Codex product behavior: fresh manual helper output, reused same-thread manual, Docs MCP fallback, or bounded uncertainty.
- Non-Codex OpenAI docs: Docs MCP search plus fetch, OpenAPI spec when API shape matters, or official-domain web fallback.
- Model-selection or migration: current `latest-model.md` or an explicitly disclosed bundled fallback.
- Implementation edits: run the repository's focused tests or at least the changed command path after updating model strings, prompts, or API parameters.
