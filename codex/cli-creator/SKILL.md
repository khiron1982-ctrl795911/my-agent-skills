...
Before scaffolding, check whether the proposed command already exists. Use the native check for the current shell:
...
Before choosing, inspect the user's machine and source material. Use whichever command form fits the shell:
...
6. Smoke test from another repo or temp directory, not only with `cargo run` or package-manager wrappers. Run `command -v <tool-name>` or `Get-Command <tool-name>`, `<tool-name> --help`, and `<tool-name> --json doctor`.
...
Before handing off a CLI, verify the exact installed binary from a different working directory:

- `--help` exits successfully and shows the main command families.
- `--json doctor` emits valid JSON with no secrets.
- At least one discovery/read command works from fixture, dry-run, or read-only live data.
- Write commands support a preview, draft, dry-run, or explicit confirmation path when the service allows it.
- Generated files, cache files, and credentials are excluded from commits unless the user explicitly wants them included.
