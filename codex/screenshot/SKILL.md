...
If the skill path is under the default user install, this is usually:

```powershell
powershell -ExecutionPolicy Bypass -File "$env:USERPROFILE\.codex\skills\screenshot\scripts\take_screenshot.ps1"
```
...
- For Codex-only visual inspection, view the saved file before describing the screen so the response is based on the captured image, not memory of the command.
